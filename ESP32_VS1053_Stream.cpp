...
#include "VS1053.h" // si ce n’est pas déjà inclus

void ESP32_VS1053_Stream::writeCodecRegister(uint8_t reg, uint16_t value) {
    _vs1053->write_register(reg, value);
}

uint16_t ESP32_VS1053_Stream::readCodecRegister(uint8_t reg) {
    return _vs1053->read_register(reg);
}
