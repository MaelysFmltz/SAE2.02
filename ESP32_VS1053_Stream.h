...
public:
    ESP32_VS1053_Stream();
    void writeCodecRegister(uint8_t reg, uint16_t value);
    uint16_t readCodecRegister(uint8_t reg);
    ~ESP32_VS1053_Stream();
    void setEarspeaker(uint8_t mode) {
      	if (_vs1053) {
            uint16_t value = _vs1053->read_register(0x00); // SCI_MODE
            value &= ~((1 << 4) | (1 << 7)); // Efface les bits EarSpeaker
            if (mode & 0x01) value |= (1 << 4); // ES0
            if (mode & 0x02) value |= (1 << 7); // ES1
            _vs1053->write_register(0x00, value); // Réécrit dans SCI_MODE
     	}
    }
